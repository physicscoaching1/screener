...
for i, each in enumerate (stocklist['SYMBOL']):
    ...
    if ((stocklist['combinedflag'].iloc[i] != stocklist2['combinedflag'].iloc[i])):
        ...
        try:
            ...
            existingdata = pd.read_csv(os.path.join(cwd, 'data', 'Annual', filename), index_col = 0)
            existingdata.index = pd.to_datetime(existingdata.index, errors='ignore', format = "%Y/%m/%d")
